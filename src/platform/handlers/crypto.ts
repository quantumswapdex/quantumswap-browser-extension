...
export default {
  async CryptoApiEncrypt(data: any) {
  ...
  async CryptoApiDecrypt(data: any) {
    const aesKey = base64ToBytes(data.key);
    const aesIV = base64ToBytes(data.iv);
  ...
  async CryptoRandomBytes(data: any) {
  ...
  async WalletEncryptJson(data: any) {
    await Initialize(null);
  ...
  async WalletDecryptJson(data: any) {
    await Initialize(null);
  ...
  async VerifyMessage(data: any) {
    await Initialize(null);
    const address = verifyMessage(data.message, data.signature);
    return { address };
  },

  async IsValidAddress(data: any) {
    await Initialize(null);
    return isAddress(data);
  },

  async ScryptDerive(data: any) {
    await Initialize(null);
