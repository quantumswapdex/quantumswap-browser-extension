...
(function () {
    ...
    async function loadAccounts(password) {
        ...
        for (var i = 0; i <= maxIndex; i++) {
            var w = await walletGetByIndex(password, i);
            if (w) accounts.push(w);
        }
        return accounts;
    }
    ...
    async function doConnect(origin, password) {
        ...
        // Prefer an already-unlocked wallet (locked path); otherwise unlock the
        // shared/default address with the entered password (ready path).
        var wallet = null;
        if (connectAccounts) {
        ...
        if (!wallet) {
            wallet = await walletGetByAddress(password, address);
        }
        if (!wallet) {
    ...
    async function doSend(method, params, password) {
        ...
        if (isCoin) {
            result = await submitSendCoins({
                ...
                privateKey: priv,
                publicKey: pub,
                gasLimit: (typeof COIN_SEND_GAS !== "undefined") ? COIN_SEND_GAS : 21000,
            ...
            result = await submitSendTokens({
                ...
                privateKey: priv,
                publicKey: pub,
                gasLimit: (typeof TOKEN_SEND_GAS !== "undefined") ? TOKEN_SEND_GAS : 84000,
                advancedSigningEnabled: fullSign
            });
        }
        ...
        // Hand off to background for confirmation polling + transactionResult,
        // then show a short confirmation and close.
        await replyBroadcast(result.txHash, wallet.address);
        hideWaitingBox();
        setStatus(t("dapp-tx-submitted", "Transaction submitted: [TRANSACTION_HASH]").replace("[TRANSACTION_HASH]", result.txHash));
    ...
    function onApprove() {
        ...
        // Connect + locked: first click only unlocks (loads accounts, shows the
        // dropdown). It does not connect yet.
        if (pendingRequest.method === "qc_requestAccounts" && connectState === "locked") {
            if (approveBtn) approveBtn.disabled = true;
            showLoadingAndExecuteAsync(t("waitWalletOpen", "Please wait while your wallet is being opened."), function () {
                unlockConnect(password).then(function () {
                    hideWaitingBox();
